...
%--------------------------------------------------------------------------------------------------------------------------------------------------------------
%SlopeFit(Using Lines to find LLambda) = Equations with LLambda (2)
%MaxLklyHd(Max Likely Hood) = Equations with llambda
%--------------------------------------------------------------------------------------------------------------------------------------------------------------
%--------------------------------------------------------------------------------------------------------------------------------------------------------------
SlopeFit = table(LLambda, Avgdlnp2,sigmaLl2,LlPos2,LlNeg2,'VariableNames',{'SlopeLlambda','SlpApproxAvge','SlpStandardDev','StndrdDevPos','StndrdDevNeg'}) %#ok<NASGU>
disp(d1)
MaxLklyHd = table(llambda,Avgdlnp,sigmaLl,LlPos,LlNeg,'VariableNames',{'ApproxLlambda','ApproxAverage','ApproxStandardDev','StndrdDevPos','StndrdDevNeg'}) %#ok<NASGU>
disp(d1)
...
legend('Histogram of MuonDecay Events',' Center of Bin vs. BinCound', 'BinRange vs. BinCount', 'BackGround Range')
hold off
...
ylabel('Number of Decays events')
grid on
legend('Muon Decay Events','h center','Best Fit');
hold on
%--------------------------------------------------------------------------------------------------------------------------------------------------------------
...
legend([h5 q5 q6 q7 Yq5LN],'Histogram of MuonDecay Events',' Center of Bin vs. BinCound', 'BinRange vs. BinCount', 'BackGround Range','LnLLambda is. DecayTime')
hold on
...
xlabel('Time in Micro-seconds (X 10^-6)')
yylabel('Number of Decays events (BinWidth 0.5MicroSecond)')
...
yCalc2 = XX2*slp2;
plot(X2,yCalc2,'ok')
legend('DecayEvents','Bin Count vs. Range','Ydata Vs. YLine','Slope & Intercept','Location','best');
...
hDf.EdgeColor = 'k';             
hold on
ylnt = scatter(DecayDomain,yln,'k');    %DecayDomain = MuD, but it's in order least>>Greatest
hold on
%Linear Fit
Xln = CentrBn2(:,1); %Interval the DecayEventData set down collum one
Yln = BinR2(:,1); %Interval the Bin Range set down collum one
format long
...
legend('Ln(N(t)) vs t');
xlabel(' Event Duration Distribution from 0.04MicroSeconds to 19.96MicroSeconds')
