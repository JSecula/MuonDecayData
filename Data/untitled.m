...
%-----------------------------------------------------------------------------------
figure('Name','BackGround for 0.5 width, 40 bin')     
p3 = polyfit(PntFiveCnt,PntFiveRng,1);
disp(p3)
Coefficients3 = table(p3, 'VariableNames',{'CountVsRange1'}) %#ok<NOPTS>
disp(d1)
 
[p4,S,mu] = polyfit(PntTwoCnt,PntTwoRng,1);
disp(p4)
Coefficients4 = table(p4, 'VariableNames',{'CountVsRange2'}) %#ok<NOPTS>
disp(d1)
p2S4 = 'PolyFit Range vs. Bin Count, 0.5MicroSeconds version 3';
disp(p2S4)
disp(S)
p2mu = 'PolyFit Range vs. Bin Count, 0.5MicroSeconds version 3';
disp(p2mu)
disp(mu)
 
disp(d1)
Ply2 = table(p3,p4,'VariableNames',{' PolyFit 3 ',' Poly Fit 4'}) %#ok<NOPTS>
disp(d1)
disp(win)
%--------------------------------
%------------PLOT----------------
%--------------------------------
fittedX3 = linspace(min(Trial_Two), max(Trial_Two),40) %#ok<NOPTS>
fittedY3 = polyval(p3, fittedX3);
disp(fitx)
disp(fittedX3)                                                 %BinCount
disp(d1)
disp(fity)
disp(fittedY3)                                                 %BinCount
disp(d1)
%--------------------------------
hold on;
subplot(2,2,1)
bar(h3.');
hold on;
plot(fittedX3, fittedY3, 'or', 'LineWidth', 3);
legend('show')
%--------------------------------
%--------------------------------
fittedX4 = linspace(min(Trial_Two), max(Trial_Two),40) %#ok<NOPTS>
fittedY4 = polyval(p4, fittedX);
fitx = 'Fitted X coordinates';
disp(fitx)
disp(fittedX4)                                                 %BinCount
disp(d1)
fity = 'Fitted Y coordinates';
disp(fity)
disp(fittedY4)                                                 %BinCount
disp(d1)
%Plot the fitted line
hold on;
%--------------------------------
...
bar(h3.');
hold on;
plot(fittedX4, fittedY4, 'or', 'LineWidth', 3);
legend('show')
%--------------------------------
hold off
disp(win)
